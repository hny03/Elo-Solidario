    ...
    <main class="container my-5">
        <div class="row g-4">
            ...
            <?php foreach ($posts as $post): ?>
                        ...
                        <div class="card-body d-flex flex-column">
                            <div class="row">
                                <div class="col-8">
                                    ...
                                    <p class="card-text">
                                        <?= htmlspecialchars($post['descricao_curta']) ?>
                                    </p>
                                </div>
                            ...
                            <div class="mt-auto pt-3">
                                <div class="d-flex gap-2 mb-2">
                                    <span class="badge-doacao"><?= htmlspecialchars($post['tipo']) ?></span>
                                    <span class="badge-presencial"><?= htmlspecialchars($post['modalidade']) ?></span>
                                </div>
                            </div>
                            <a href="#" class="stretched-link" data-bs-toggle="modal"
                                data-bs-target="#modalAnuncio<?= $post['id'] ?>">
                ...
                <div class="modal fade" id="modalAnuncio<?= $post['id'] ?>" tabindex="-1">
                            ...
                            <div class="modal-body">
                                <div class="mb-4">
                                    <dl class="row">
                                        ...
                                        <dd class="col-sm-8"><?= htmlspecialchars($post['nomeOrganizacao']) ?></dd>
                                        <dt class="col-sm-4"><i class="bi bi-heart"></i> Causa Principal:</dt>
                                        <dd class="col-sm-8"><?= htmlspecialchars($post['causa_principal']) ?></dd>

                                        <dt class="col-sm-4"><i class="bi bi-geo-alt"></i> Localização:</dt>
                                        <dd class="col-sm-8"><?= htmlspecialchars($post['localizacao']) ?></dd>
                                    </dl>
                                </div>
                                <hr>
                                <h6>Descrição:</h6>
                                <p><?= nl2br(htmlspecialchars($post['descricao_completa'])) ?></p>
                            </div>
                            <div class="modal-footer">
                                <?php if ($post['tipo'] === 'Doação' && !empty($post['contato'])): ?>
                                    <p class="mb-0"><strong>PIX para doação:
                                            <?= htmlspecialchars($post['contato']) ?></strong></p>
                                <?php elseif ($post['tipo'] === 'Voluntariado' && !empty($post['contato'])): ?>
                                    <p class="mb-0"><strong>
                                            <?= htmlspecialchars($post['contato']) ?></strong></p>
                                <?php endif; ?>
